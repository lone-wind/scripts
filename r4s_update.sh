...
#工作目录
work_path () {
    echo '\e[92m请手动指定存放固件的路径\e[0m' && df -h
    echo '\e[91m千万不要使用/overlay\e[0m'
    echo '\e[92m2G内存以上可使用/tmp\e[0m'
    read -p "输入带有“/”的绝对路径回车 " work_path
    echo "你选择的文件夹路径为${work_path}"
}
#版本选择
version_choose () {
    echo '\e[92m输入对应数字选择版本或退出\e[0m'
    echo "0 --- Exit退出"
    echo "1 --- Docker_高大全"
    echo "2 --- Stable_稳定版"
    echo "3 --- Formal_正式版"
    ...
    case $version_num in
        0)
            echo '\e[91m退出脚本，升级结束\e[0m'
            exit;
            ;;
        1)
            echo '\e[92m已选择Docker_高大全\e[0m'
            ;;
        2)
            echo '\e[92m已选择Stable_稳定版\e[0m'
            ;;
        3)
            echo '\e[92m已选择Formal_正式版\e[0m'
            ;;
        *)
            echo '\e[91m非法输入,请输入数字[0-3]\e[0m'
            version_choose
            ;;
    esac
}
...
format_choose () {
    ...
    case $format_num in
        0)
            ...
            ;;
        1)
            echo '\e[92m已选择EXT4格式\e[0m'
            format=ext4
            ;;
        2)
            echo '\e[92m已选择Squashfs格式\e[0m'
            format=squashfs
            ;;
        *)
            echo '\e[91m非法输入,请输入数字[0-2]\e[0m'
            format_choose
            ;;
    esac
}
...
exist_judge () {
    if [ -f openwrt-rockchip-armv8-friendlyarm_nanopi-r4s-${format}-sysupgrade.img.gz ]; then
        echo '\e[92m固件已下载\e[0m'
        echo `(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)`-Lean$version_num
        version_skip
    elif [ $days == 21 ]; then
        echo '\e[91m未找到合适固件，脚本退出\e[0m'
        exit;
    else
        echo '\e[91m当前固件不存在，寻找前一天的固件\e[0m'
        download_file
    fi
}
#跳过固件
version_skip () {
    ...
    case $skip in
        [yY][eE][sS]|[yY])
            ...
            ;;
        [nN][oO]|[nN])
            echo '\e[91m寻找前一天的固件\e[0m'
            download_file
            ;;
        [eE][xX][iI][tT]|[eE])
            echo '\e[91m取消固件下载，退出升级\e[0m'
            clean_up
            exit;
            ;;
        *)
            echo '\e[91m请输入[Y/N]进行确认，输入[E]退出\e[0m'
            version_skip
            ;;
    esac
}
...
version_confirm () {
    ...
    case $confirm in
        [yY][eE][sS]|[yY])
            echo '\e[92m已确认升级\e[0m'
            ;;
        [nN][oO]|[nN])
            echo '\e[91m已确认退出\e[0m'
            clean_up
            exit;
            ;;
        *)
            echo '\e[91m请输入[Y/N]进行确认\e[0m'
            version_confirm
            ;;
    esac
}
