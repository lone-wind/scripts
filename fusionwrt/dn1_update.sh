...
version_choose () {
    ...
    case $version_num in
        0)
            ...
            ;;
        1)
            echo -e '\e[92m已选择Docker_容器版\e[0m' && version_num=3
            ;;
        2)
            echo -e '\e[92m已选择Stable_稳定版\e[0m' && version_num=4
            ;;
        3)
            echo -e '\e[92m已选择Formal_正式版\e[0m' && version_num=8
            ;;
        *)
...
#刷写系统
dd_system () {
    echo -e '\e[92m开始刷入系统\e[0m'
    dd if=${img_path}/${firmware_id} of=/dev/${hd_id} bs=1M
    echo -e '\e[92m刷入系统完毕，请手动断电再上电\e[0m'
}
#系统更新
update_firmware () {
