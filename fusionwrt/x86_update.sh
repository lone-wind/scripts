...
search_file () {
    ...
    wget -q ${proxy_url}/${repo_url}/download/$(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)-Lean3/${version_num}-sha256sums
    exist_judge
}
#存在判断
exist_judge () {
    if [ -f ${version_num}-sha256sums ]; then
        echo -e '\e[92m已找到当前日期的固件\e[0m' && echo `(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)`-Lean3
        firmware_confirm
    elif [ $days == 21 ]; then
...
firmware_confirm () {
    ...
    case $skip in
        [yY][eE][sS]|[yY])
            ...
            wget ${proxy_url}/${repo_url}/download/$(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)-Lean3/${firmware_id}.gz
            ;;
        [nN][oO]|[nN])
