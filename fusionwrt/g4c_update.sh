...
#仓库选择
repo_set () {
    repo_url=https://github.com/DHDAXCW/guangmiao-g4c/releases
    firmware_id=openwrt-rockchip-armv8-huake-cloud_guangmiao-g4c-${format}-sysupgrade.img
}
#寻找固件
search_file () {
    cd ${work_path} && clean_up && days=$(($days+1))
    #echo `(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)`
    wge -q ${repo_url}/download/$(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)-Lean${version_num}/sha256sums
    exist_judge
}
#存在判断
exist_judge () {
