...
repo_set () {
    ...
    repo_url=https://github.com/DHDAXCW/NanoPi-R5S/releases
    firmware_id=openwrt-rockchip-armv8-friendlyarm_nanopi-r5s-${format}-sysupgrade.img
}
#寻找固件
search_file () {
...
update_firmware () {
    ...
    if [ $real_mem -ge $mini_mem ]; then 
        work_path=/tmp && version_num=3
        echo -e '\e[92m已选择：正式版\e[0m'
